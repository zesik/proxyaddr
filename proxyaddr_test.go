...
func TestInitSingleCIDR(t *testing.T) {
	...
	assert.Equal(t, net.IPNet{[]byte{0x0a, 0x0a, 0x0a, 0x00}, []byte{0xff, 0xff, 0xff, 0x00}},
		*pa.trustedProxies[0])
}
...
func TestInitMultipleCIDR(t *testing.T) {
	...
	assert.Equal(t, net.IPNet{net.IP{0x0a, 0x0a, 0x0a, 0x0}, net.IPMask{0xff, 0xff, 0xff, 0x0}},
		*pa.trustedProxies[0])
	assert.Equal(t, net.IPNet{net.IP{0xac, 0x1b, 0x50, 0x0}, net.IPMask{0xff, 0xff, 0xf0, 0x0}},
		*pa.trustedProxies[1])
}
...
func TestInitName(t *testing.T) {
	...
	assert.NotNil(t, pa.trustedProxies)
	assert.Len(t, pa.trustedProxies, 2)
	assert.Equal(t, net.IPNet{net.IP{0x7f, 0x0, 0x0, 0x0}, net.IPMask{0xff, 0x0, 0x0, 0x0}},
		*pa.trustedProxies[0])
	assert.Equal(t,
		net.IPNet{
			net.IP{0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x1},
			net.IPMask{0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
...
func TestInitMixedNameAndCIDR(t *testing.T) {
	...
	assert.Equal(t, net.IPNet{net.IP{0xc0, 0xa8, 0x75, 0x0}, net.IPMask{0xff, 0xff, 0xff, 0x00}},
		*pa.trustedProxies[0])
	assert.Equal(t, net.IPNet{net.IP{0x99, 0xdc, 0x22, 0x23}, net.IPMask{0xff, 0xff, 0xff, 0xff}},
		*pa.trustedProxies[3])
}
